  ...
  return (
    <div className="mx-4 mt-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs text-gray-400 font-semibold">{t('FamilyBar.operating')}</span>
        ...
        >
          {t('FamilyBar.bringFamily')}
        </button>
      </div>

      {open && (
        <div className="card mt-2 space-y-2">
          <p className="text-xs text-gray-500 font-semibold">{t('FamilyBar.addHint')}</p>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t('FamilyBar.namePlaceholder')}
            className="w-full border-2 border-gray-200 rounded-2xl px-3 py-2 text-sm
              focus:outline-none focus:border-brand-pink"
          />
          {/* 可愛頭像 */}
          <div>
            <span className="text-xs font-bold text-gray-500">{t('FamilyBar.pickAvatar')}</span>
          ...
          >
            {adding ? t('FamilyBar.submitting') : t('FamilyBar.submit')}
          </button>
        </div>
      )}
